...
public class AttendanceController {
    ...
    // 기존 @GetMapping("/check")는 삭제하거나 다른 목적으로 사용해주세요.
    // '오늘 출석하기' 버튼이 POST 요청을 사용하므로, 이 GET 매핑은 이제 필요 없습니다.
    // @GetMapping("/check")
    ...
    public String viewAttendanceCalendar(HttpSession session, Model model) {
        ...
        if (userNo != null) {
            ...
            // List<LocalDate> attendanceDates = attendanceService.getAttendanceDates(userNo.intValue()); // 필요 없음
            // model.addAttribute("attendanceDates", attendanceDates); // 필요 없음
        } else {
    ...
    public ResponseEntity<List<String>> getAttendanceDatesJson(HttpSession session) {
        ...
        // ✅ IMPORTANT: 여기서 attendanceService.checkAttendance 호출 제거!
        // 이 호출은 '오늘 출석하기' 버튼을 눌렀을 때만 이루어져야 합니다.
        // attendanceService.checkAttendance(userNo.intValue()); // 이 줄을 삭제했습니다.
